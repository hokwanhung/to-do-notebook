...
def is_safe(state): 
    ...
    return ( (state[BOAT_POSITION] == LEFT and state[LEFT][Goat] == 1) or
             ...
             (state[BOAT_POSITION] == RIGHT and (state[LEFT][Wolf] != state[LEFT][Goat] and state[LEFT][Goat] != state[LEFT][Cabbage]))
            )
...
if path: 
    ...
    for p in path: #path = bfs_search(initial_state)
        ...
        if side==LEFT:
            ...
            boat_l = "      ";boat_r = "#BOAT#"
        print(" Wolf {}  Goat {}  Cabbage {} {} |~~~~~| {}  Wolf {}  Goat {} Cabbage {}" .format(w_left, g_left, c_left, boat_l, boat_r, w_right, g_right, c_right))
