public class controller {
    ...
    /*
     * Set up basic connections between controller, model and view (as well as admin).
     */
    ...
    public void buyControl(){
        v.buyOrPass(); // perform the buyOrPass() from view, which disables the DICE and enables BUY or PASS
        System.out.println("change status");
    }
    
    public void option(int o){
        buyControl(); // call the above buyControl() function
        if (o == 1){ // if the option = 1, then the user clicks on BUY button and thus perform processBuy() from model.
            m.processBuy();
    ...
    public void changeStatus(int pl){
        m.Status(pl); // use controller to connect back to model
    }
}
